...
public class BackupUtil {
    ...
    public static void doBackup(boolean uploadToServer) {
        ...
        eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.backup.starting"));

        // do not backup when plugin is disabled
        if (!eBackup.getPlugin().isEnabled()) {
            eBackup.getPlugin().getLogger().warning(eBackup.getPlugin().messages.getString("logs.backup.plugin_disabled"));
            return;
        }
        ...
        try {
            ...
            for (World w : Bukkit.getWorlds()) {
                ...
                eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.backup.backing_up_world").replace("{world}", w.getName()).replace("{path}", worldPath));
            ...
            // dfs all other files
            eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.backup.backing_up_other"));
        ...
        eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.backup.complete"));
    }
    ...
    private static void uploadTask(String fileName, boolean testing) {
        if (eBackup.getPlugin().isInUpload.get()) {
            eBackup.getPlugin().getLogger().warning(eBackup.getPlugin().messages.getString("logs.upload.scheduled_skipped"));
            return;
        }

        boolean isSFTP = eBackup.getPlugin().ftpType.equals("sftp"), isFTP = eBackup.getPlugin().ftpType.equals("ftp");
        if (!isSFTP && !isFTP) {
            eBackup.getPlugin().getLogger().warning(eBackup.getPlugin().messages.getString("logs.upload.invalid_type"));
            return;
        }

        eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.upload.starting").replace("{file}", fileName).replace("{type}", isSFTP ? "SFTP" : "FTP"));
        Bukkit.getScheduler().runTaskAsynchronously(eBackup.getPlugin(), () -> {
            try {
                ...
                if (testing) {
                    f.delete();
                    eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.upload.test_successful"));
                } else {
                    eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.upload.successful").replace("{file}", fileName));
                }
            } catch (Exception e) {
                e.printStackTrace();
                eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.upload.failed").replace("{file}", fileName));
            } finally {
    ...
    private static void deleteAfterUpload(File f) {
        if (eBackup.getPlugin().deleteAfterUpload) {
            Bukkit.getScheduler().runTaskAsynchronously(eBackup.getPlugin(), () -> {
                if (f.delete()) {
                    eBackup.getPlugin().getLogger().info(eBackup.getPlugin().messages.getString("logs.delete_after_upload.successful").replace("{file}", f.getName()));
                } else {
                    eBackup.getPlugin().getLogger().warning(eBackup.getPlugin().messages.getString("logs.delete_after_upload.failed").replace("{file}", f.getName()));
                }
            });
        }
    }
    ...
    private static void zipFile(File fileToZip, String fileName, ZipOutputStream zipOut, Set<String> visitedFiles) throws IOException {
        ...
        if (fileToZip.isDirectory()) { // if it's a directory, recursively search
            ...
            try {
                ...
                eBackup.getPlugin().getLogger().warning(eBackup.getPlugin().messages.getString("logs.backup_file.error").replace("{file}", fileName).replace("{message}", e.getMessage()));
            }
        }
    }
}
